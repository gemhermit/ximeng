...
const termsContent: { zh: TermsSection[]; en: TermsSection[] } = {
  zh: [
    {
      title: '引言',
      paragraphs: [
...
const Terms: React.FC = () => {
  ...
  const keywords = strings(language, 'terms.keywords');
  const sections = termsContent[language];
    ...
    <div className="min-h-screen bg-slate-950">
      <Seo
        ...
        structuredData={buildBreadcrumbSchema([
          { name: t('terms.home'), path: route('/') },
          { name: t('terms.title'), path: route('/terms') },
        ])}
      />
      <PageHeader title={t('terms.title')} subtitle={t('terms.subtitle')} gradient="from-blue-400 via-cyan-400 to-teal-400" />
      ...
      <div className="container mx-auto px-6 py-20">
        <div className="max-w-4xl mx-auto bg-slate-900/50 backdrop-blur-md rounded-2xl p-8 border border-white/10">
          <div className="mb-12 p-4 bg-blue-900/20 border border-blue-500/30 rounded-lg">
            <p className="text-sm text-gray-300">{t('terms.version')}</p>
          </div>
          ...
          <section>
            <h2 className="text-2xl font-bold text-white mb-4">{t('terms.contactTitle')}</h2>
            <p className="text-gray-300 leading-relaxed">{t('terms.contactIntro')}</p>
            <div className="mt-4 p-4 bg-gray-800/50 rounded-lg">
              <p className="text-gray-300">{t('terms.emailLabel')}<a href="mailto:echo@ximengtech.cn" className="text-blue-400 hover:underline">echo@ximengtech.cn</a></p>
              <p className="text-gray-300 mt-2">{t('terms.addressLabel')}{t('terms.address')}</p>
            </div>
          </section>
        </div>
      </div>
