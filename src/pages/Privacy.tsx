...
const privacyContent: { zh: LegalSection[]; en: LegalSection[] } = {
  zh: [
    {
      title: '引言',
      paragraphs: [
    ...
    },
    {
      ...
      subsections: [{ title: '', paragraphs: [<>要行使这些权利，请通过<Link to="/contact" className="text-blue-400 hover:underline">联系我们</Link>页面提交请求。我们将在适用法律要求的时间内响应您的请求。</>] }],
    },
      ...
      subsections: [
        {
          title: '',
          paragraphs: [
            <>To exercise these rights, please submit a request through the <Link to="/contact" className="text-blue-400 hover:underline">Contact Us</Link> page. We will respond within the timeframe required by applicable law.</>,
          ],
        },
      ],
    },
...
const Privacy: React.FC = () => {
  ...
  const keywords = strings(language, 'privacy.keywords');
    ...
    <div className="min-h-screen bg-slate-950">
      <Seo
        ...
        structuredData={buildBreadcrumbSchema([
          { name: t('privacy.home'), path: route('/') },
          { name: t('privacy.title'), path: route('/privacy') },
        ])}
      />
      <PageHeader title={t('privacy.title')} subtitle={t('privacy.subtitle')} gradient="from-blue-400 via-cyan-400 to-teal-400" />
      ...
      <div className="container mx-auto px-6 py-20">
        <div className="max-w-4xl mx-auto bg-slate-900/50 backdrop-blur-md rounded-2xl p-8 border border-white/10">
          <div className="mb-12 p-4 bg-blue-900/20 border border-blue-500/30 rounded-lg">
            <p className="text-sm text-gray-300">{t('privacy.version')}</p>
          </div>
          ...
          <section>
            <h2 className="text-2xl font-bold text-white mb-4">{t('privacy.contactTitle')}</h2>
            <p className="text-gray-300 leading-relaxed">{t('privacy.contactIntro')}</p>
            <div className="mt-4 p-4 bg-gray-800/50 rounded-lg">
              <p className="text-gray-300">{t('privacy.emailLabel')}<a href="mailto:echo@ximengtech.cn" className="text-blue-400 hover:underline">echo@ximengtech.cn</a></p>
              <p className="text-gray-300 mt-2">{t('privacy.addressLabel')}{t('privacy.address')}</p>
            </div>
          </section>
        </div>
      </div>
