...
type MegaItem = { href: string; color: string; title: string; desc: string };
type MegaColumn = { title: string; dot: string; items: MegaItem[] };

const megaData = (language: 'zh' | 'en'): MegaColumn[] => {
  const columns = strings(language, 'nav.mega') as unknown as MegaColumn[];
  const dotColors = language === 'en'
    ? ['bg-blue-500', 'bg-cyan-500', 'bg-purple-500', 'bg-fuchsia-500']
    : ['bg-blue-500', 'bg-cyan-500', 'bg-purple-500', 'bg-fuchsia-500'];
  return columns.map((column, idx) => ({ ...column, dot: dotColors[idx] }));
};
    ...
    <>
        ...
        <div className={`px-4 md:px-8 ${useSolidStyle ? 'h-16' : 'h-20'} flex justify-between items-center transition-all duration-300`}>
            ...
            <Link to={route('/')} className="flex items-center gap-2 text-xl font-bold tracking-tighter hoverable z-50" onClick={closeMenu}>
               <img src="/images/logo.png" alt={t('nav.brand')} className="h-6 w-auto" />
               <span>{t('nav.brand')}</span>
            </Link>

            <nav className="hidden md:flex items-center space-x-8">
                <Link to={route('/')} className={navLinkClasses}>{t('nav.home')}<span className={underlineClasses}></span></Link>

                <div className="group relative h-full flex items-center">
                    <Link to={route('/solutions')} className={navLinkClasses + " flex items-center gap-1"}>
                        {t('nav.solutions')} <i className="fas fa-chevron-down text-[10px] opacity-50 group-hover:rotate-180 transition-transform"></i>
                ...
                <Link to={route('/cases')} className={navLinkClasses}>{t('nav.cases')}<span className={underlineClasses}></span></Link>
                <Link to={route('/careers')} className={navLinkClasses}>{t('nav.careers')}<span className={underlineClasses}></span></Link>
            </nav>
        ...
        <div className={`fixed inset-0 bg-slate-950/98 backdrop-blur-lg z-40 flex flex-col justify-center px-8 transition-all duration-300 md:hidden ${isMobileMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}>
             ...
             <div className="space-y-6 relative z-10">
                <Link to={route('/')} onClick={closeMenu} className="block text-3xl font-bold hover:text-blue-500 transition-colors">{t('nav.home')}</Link>
                <Link to={route('/solutions')} onClick={closeMenu} className="block text-3xl font-bold hover:text-blue-500 transition-colors">{t('nav.mobileSolutions')}</Link>
                <Link to={route('/cases')} onClick={closeMenu} className="block text-3xl font-bold hover:text-blue-500 transition-colors">{t('nav.cases')}</Link>
                <Link to={route('/careers')} onClick={closeMenu} className="block text-3xl font-bold hover:text-blue-500 transition-colors">{t('nav.careers')}</Link>

                <div className="pt-8 border-t border-white/10">
                    <Link to={route('/contact')} onClick={closeMenu}>
                        <button className="w-full py-4 rounded-full bg-blue-600 text-white font-bold text-lg hover:bg-blue-700 transition-all">{t('nav.contactCta')}</button>
                    </Link>
                    <Link
                      to={switchTo(isEnglish ? 'zh' : 'en')}
                      onClick={closeMenu}
