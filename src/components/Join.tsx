    ...
    <section id="join" className="py-24 md:py-32 relative bg-slate-900/30">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-gray-400 mt-4">{text.subtitle}</p>
        </div>
        ...
        <div className="max-w-6xl mx-auto">
           <h3 className="text-xl font-bold text-center mb-8"><span className="border-b-4 border-purple-500 pb-2">{text.hot}</span></h3>
           <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {jobsData.map((job, i) => (
                  <Link to={route(`/careers/${job.id}`)} key={i} className={`group flex flex-col justify-between p-6 rounded-xl bg-white/5 hover:bg-${job.color}-600 hover:scale-[1.02] transition-all duration-300 hoverable cursor-pointer border border-white/5 shadow-lg`}>
                     <div className="mb-4">
                     ...
                     <div className="flex justify-between items-center mt-auto pt-4 border-t border-white/10 group-hover:border-white/20">
                        <span className="text-xs text-gray-500 group-hover:text-white">{text.type}</span>
