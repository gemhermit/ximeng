...
const Hero: React.FC = () => {
  ...
  const bodyLines = t('hero.body').split('\n');
      ...
      <div className="container mx-auto px-6 relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center h-full">
        ...
        <div className="space-y-6 md:space-y-8 text-center lg:text-left relative">
          ...
          <h1 className="text-5xl md:text-7xl lg:text-8xl font-bold leading-[1.1] tracking-tighter">
            <div className="overflow-hidden"><span className="block reveal-heading text-white">{t('hero.headlineTop')}</span></div>
          ...
          <div className="pt-12 flex gap-8 justify-center lg:justify-start opacity-60 reveal-text">
             <div>
                ...
                <div className="text-xs uppercase tracking-widest">{t('hero.stats.nodesLabel')}</div>
             </div>
             <div className="w-px h-10 bg-white/20"></div>
             <div>
                <div className="text-2xl font-bold font-mono">0.02s</div>
                <div className="text-xs uppercase tracking-widest">{t('hero.stats.latencyLabel')}</div>
             </div>
          </div>
        </div>
